...
export async function generateMetadata({ params }: Props) {
  ...
  if (!davetiye) return { title: "Davetiye Bulunamadi" };
...
export default async function DavetiyeSayfasi({ params }: Props) {
  ...
  const aktifRenk = (davetiye as any).ozelRenk || sablon.renk;
  const aktifFont =
    (davetiye as any).font === "font-serif"
      ? "Georgia, serif"
      : (davetiye as any).font === "font-mono"
      ? "monospace"
  ...
  const etiketler: Record<string, string> = {
    dugun: "Dugun Davetiyesi", nisan: "Nisan Davetiyesi",
    dogumgunu: "Dogum Gunu Partisi", sunnet: "Sunnet Toreni",
    kina: "Kina Gecesi", kurumsal: "Etkinlik Davetiyesi", diger: "Davetiye",
  };
          ...
          <FadeUp delay={0.6}>
            <div className="rounded-2xl p-5 mb-4 text-center border" style={{ backgroundColor: aktifRenk + "15", borderColor: aktifRenk + "30" }}>
              <p className="text-xs font-medium uppercase tracking-wide mb-3" style={{ color: aktifRenk }}>
                Etkinlige Kalan Sure
              </p>
              <div className="flex justify-center gap-6">
                <div>
                  <p className="text-3xl font-bold" style={{ color: aktifRenk }}>{gun}</p>
                  <p className="text-xs text-gray-400 mt-0.5">Gun</p>
                </div>
