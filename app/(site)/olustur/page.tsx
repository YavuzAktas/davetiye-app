...
function OlusturIcerigi() {
  ...
  const [basarili, setBasarili]     = useState<{ slug: string; baslik: string } | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setBasarili({ slug: data.slug, baslik: gonderilecekBaslik });
  ...
  if (basarili) {
    return <BasariEkrani slug={basarili.slug} baslik={basarili.baslik} />;
  }
...
/* ── Davetiye oluşturuldu ekranı ── */
function BasariEkrani({ slug, baslik }: { slug: string; baslik: string }) {
  const router = useRouter();
  const [kopyalandi, setKopyalandi] = useState(false);

  const url = typeof window !== "undefined"
    ? `${window.location.origin}/davetiye/${slug}`
    : `/davetiye/${slug}`;
  ...
  const whatsapp = `https://wa.me/?text=${encodeURIComponent(`${baslik} davetiyesi: ${url}`)}`;

  return (
    <div className="min-h-screen bg-linear-to-b from-purple-50 to-white flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden animate-[fadeSlideUp_0.4s_ease]">

        {/* Başlık bölümü */}
        <div className="bg-linear-to-br from-purple-600 to-violet-700 px-6 py-10 text-center">
          <div className="w-20 h-20 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-5 animate-[pop_0.5s_ease_0.1s_both]">
            <svg className="w-10 h-10 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <h1 className="text-2xl font-bold text-white mb-1">Davetiyeniz Hazır! 🎉</h1>
          <p className="text-purple-200 text-sm">{baslik}</p>
        </div>

        <div className="p-6 space-y-4">
          {/* Link kutusu */}
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Davetiye Linki</p>
          ...
          <a href={whatsapp} target="_blank" rel="noopener noreferrer"
            className="flex items-center justify-center gap-2.5 w-full bg-[#25D366] text-white py-3.5 rounded-xl font-semibold text-sm hover:bg-[#1ebe5a] active:scale-[0.98] transition-all">
          ...
          <button onClick={() => router.push(`/davetiye/${slug}`)}
            className="w-full bg-purple-600 text-white py-3.5 rounded-xl font-semibold text-sm hover:bg-purple-700 active:scale-[0.98] transition-all">
