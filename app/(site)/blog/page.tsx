...
const CAT: Record<string, { bg: string; color: string }> = {
  "Rehber": { bg: "rgba(124,58,237,0.12)", color: "#7c3aed" },
              ...
              <Link href={`/blog/${featured.slug}`} style={{ textDecoration:"none", display:"block" }}>
                <div className="blog-card" style={{
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
                    <span style={{
                      background: CAT[featured.category]?.bg ?? "rgba(124,58,237,0.1)",
                      color:      CAT[featured.category]?.color ?? "#7c3aed",
          ...
          {rest.length > 0 && (
            <>
              ...
              <div style={{
                ...
                {rest.map(post => {
                  const cat = CAT[post.category] ?? { bg:"rgba(124,58,237,0.1)", color:"#7c3aed" };
                  return (
                    <Link key={post.slug} href={`/blog/${post.slug}`} style={{ textDecoration:"none" }}>
                      <div className="blog-grid-card" style={{
                        background:"#fff", border:"1px solid #f3f4f6",
                        borderRadius:20, padding:"28px 28px 24px", height:"100%",
                      }}>
