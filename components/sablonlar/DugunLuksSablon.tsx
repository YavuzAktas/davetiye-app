      ...
      <section id="katilim" style={{ padding:"80px 24px", background:BG_DARK }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          <div style={{
            ...
            <RsvpFormKrem davetiyeId={davetiye.id} bg={BG} gold={GOLD} cream={CREAM} spotifyAktif={davetiye.spotifyAktif}/>
          </div>
        </div>
      </section>
          ...
          {seciliSarki ? (
            <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:`1px solid ${gold}50` }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <p style={{ fontFamily:"var(--font-cormorant),serif", fontSize:11, color:"#7A6040", margin:0 }}>{seciliSarki.sanatci}</p>
              </div>
              <button type="button" onClick={() => { setSeciliSarki(null); setSarkiSorgu(""); }} style={{ background:"none", border:"none", cursor:"pointer", color:"#7A6040", fontSize:18, padding:0 }}>×</button>
            </div>
          ) : (
            <div style={{ position:"relative" }}>
              ...
              {sarkiAraniyor && <span style={{ position:"absolute", right:0, top:"50%", transform:"translateY(-50%)", fontSize:11, color:"#7A6040" }}>...</span>}
                  ...
                  {sarkiSonuclar.map(s => (
                    <button key={s.id} type="button" onClick={() => { setSeciliSarki(s); setSarkiSorgu(""); setSarkiSonuclar([]); }}
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <p style={{ fontFamily:"var(--font-cormorant),serif", fontSize:11, color:"#7A6040", margin:0 }}>{s.sanatci}</p>
                      </div>
                    </button>
                  ))}
                </div>
