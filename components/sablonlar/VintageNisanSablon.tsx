...
const BG       = "#FDFAF5";   // sıcak krem
const BG_SOFT  = "#F2EAD8";   // yumuşak krem
const BG_CARD  = "#FFFDF8";   // kart arka planı
const LILAC    = "#7A52A0";   // sümbül ana
const LILAC_MD = "#A07CC4";   // orta lilas
const LILAC_LT = "#C8AEDD";   // açık lilas
const PINK     = "#C07080";   // gül pembe
const PINK_LT  = "#DFA8B8";   // açık pembe
const GREEN    = "#4C7852";   // bahçe yeşili
const GREEN_LT = "#7AAA82";   // açık yeşil
const GOLD     = "#C8A240";   // altın
const WARM     = "#3E2C18";   // derin kahve (metin)
const WARM_MD  = "#7A5838";   // orta kahve
...
function WisteriaBotanical({ side = "left", opacity = 1 }: { side?: "left" | "right"; opacity?: number }) {
  return (
    <svg width="130" height="250" viewBox="0 0 130 250" fill="none"
      ...
      <ellipse cx="84" cy="41" rx="2.5" ry="5" fill={LILAC_LT} opacity="0.38" />
      ...
      <ellipse cx="58" cy="27" rx="3" ry="5" fill={LILAC_MD} opacity="0.44" />

      {/* Wisteria salkımları — dal 2 */}
      ...
      <ellipse cx="86" cy="73" rx="2.8" ry="5.5" fill={LILAC_LT} opacity="0.36" />

      {/* Wisteria salkımları — dal 3 */}
      ...
      <ellipse cx="88" cy="103" rx="2.5" ry="5.5" fill={LILAC_MD} opacity="0.4" />

      {/* Wisteria salkımları — dal 4 */}
      ...
      <ellipse cx="76" cy="127" rx="2.8" ry="6" fill={LILAC} opacity="0.44" />

      {/* Wisteria salkımları — dal 5 */}
      ...
      <ellipse cx="70" cy="159" rx="2.5" ry="5.5" fill={LILAC_MD} opacity="0.38" />

      {/* Sol yapraklar (ana dal) */}
      ...
      <circle cx="16" cy="198" r="4" fill={PINK_LT} opacity="0.36" />
      {/* Küçük yapraklar çiçek yanında */}
...
/* ─── Çelenk Mühür ─── */
function FloralSeal({ size = 190, onClick }: { size?: number; onClick?: () => void }) {
  ...
  return (
    <div onClick={handle} style={{
      ...
      <svg width={size} height={size} viewBox="0 0 200 200" fill="none">
        {/* Dış çelenk: wisteria + yapraklar dönüşümlü */}
        {Array.from({ length: 20 }, (_, i) => {
          ...
          if (i % 3 === 2) {
            return (
              <ellipse key={i} cx={cx} cy={cy} rx="4" ry="7.5" fill={GREEN} opacity="0.5"
                transform={`rotate(${rot}, ${cx}, ${cy})`} />
            );
          }
          return (
            <g key={i}>
              <ellipse cx={cx} cy={cy} rx="5" ry="9.5"
                fill={i % 2 === 0 ? LILAC : LILAC_MD} opacity="0.72"
                transform={`rotate(${rot}, ${cx}, ${cy})`} />
              <ellipse cx={cx} cy={cy + Math.cos(angle) * 7} rx="3.5" ry="7"
                fill={LILAC_LT} opacity="0.45"
                transform={`rotate(${rot}, ${cx + Math.cos(angle) * 7}, ${cy + Math.sin(angle) * 7})`} />
            </g>
          );
        })}
        {/* Pembe vurgu çiçekler */}
        {[0, 72, 144, 216, 288].map((deg, i) => {
          ...
          return (
            <g key={i}>
              <circle cx={cx} cy={cy} r="7" fill={PINK} opacity="0.72" />
              <circle cx={cx} cy={cy} r="3.5" fill={PINK_LT} opacity="0.9" />
              <circle cx={cx} cy={cy} r="1.8" fill={GOLD} opacity="0.85" />
            </g>
          );
        })}
        {/* Dairesel çizgi */}
        <circle cx="100" cy="100" r="68" stroke={GOLD} strokeWidth="0.8" fill="none" opacity="0.5"
          strokeDasharray="3 3" />
        <circle cx="100" cy="100" r="62" stroke={LILAC_LT} strokeWidth="0.5" fill="none" opacity="0.45" />
        {/* Krem merkez */}
        <circle cx="100" cy="100" r="57" fill={BG} opacity="0.97" />
        {/* İç yeşil çapraz yapraklar */}
        <path d="M100 52 C103 65 100 75 100 82 C100 75 97 65 100 52Z" fill={GREEN} opacity="0.4" />
        <path d="M100 148 C103 135 100 125 100 118 C100 125 97 135 100 148Z" fill={GREEN} opacity="0.4" />
        <path d="M52 100 C65 97 75 100 82 100 C75 100 65 103 52 100Z" fill={GREEN} opacity="0.4" />
        <path d="M148 100 C135 97 125 100 118 100 C125 100 135 103 148 100Z" fill={GREEN} opacity="0.4" />
        {/* Merkez gül */}
        {[0, 60, 120, 180, 240, 300].map((deg, i) => {
          const rad = (deg - 90) * Math.PI / 180;
          const px = 100 + Math.cos(rad) * 11;
          const py = 100 + Math.sin(rad) * 11;
          return <ellipse key={i} cx={px} cy={py} rx="4.8" ry="7.5" fill={PINK}
            opacity={0.28 + (i % 2) * 0.08} transform={`rotate(${deg}, ${px}, ${py})`} />;
        })}
        <circle cx="100" cy="100" r="6.5" fill={PINK_LT} opacity="0.75" />
        <circle cx="100" cy="100" r="3" fill={GOLD} opacity="0.9" />
        {/* Alt yazı */}
        <text x="100" y="138" textAnchor="middle" fontFamily="Georgia,serif" fontSize="8.5"
          fill={LILAC} opacity="0.65" letterSpacing="2.5">✦ dokun ✦</text>
      </svg>
    </div>
  );
}
...
function DressCodeSection({ dressKod, dressKodRenkler }: { dressKod: string; dressKodRenkler: string | null }) {
  ...
  return (
    <section style={{ padding: "80px 24px 90px", textAlign: "center", background: BG_SOFT }}>
      <p style={{ fontFamily: "var(--font-cormorant),serif", fontSize: 10, letterSpacing: "0.36em", color: LILAC, textTransform: "uppercase", marginBottom: 12 }}>
        Dress Code
      </p>
      <p style={{ fontFamily: "var(--font-dancing),cursive", fontSize: "clamp(2.4rem,7vw,3.6rem)", color: WARM, lineHeight: 1.15, marginBottom: 8 }}>
        Gecenin Renkleri
      </p>
      <p style={{ fontFamily: "var(--font-cormorant),serif", fontSize: "clamp(1rem,3vw,1.3rem)", fontStyle: "italic", color: LILAC_MD, letterSpacing: "0.08em", marginBottom: 24 }}>
        {dressKod}
      </p>
    ...
    <>
      <style>{`
        @keyframes sealFloat {
          ...
          50%       { transform: rotate(1deg) translateY(-9px) scale(1.02); }
        }
        @keyframes sealTap {
          0%   { transform: scale(1); }
      ...
      {!acildi && (
        <>
          {/* ── Bahçe kapısı giriş sayfası ── */}
          <div className="min-h-screen flex flex-col relative overflow-hidden select-none"
            style={{ background: BG }}>

            {/* Yumuşak ışık gradyanı — golden hour */}
            <div style={{
              position: "absolute", inset: 0, pointerEvents: "none",
              background: `radial-gradient(ellipse 80% 60% at 50% 100%, rgba(200,162,64,0.12) 0%, transparent 70%),
                           radial-gradient(ellipse 60% 40% at 30% 0%,  rgba(122,82,160,0.08) 0%, transparent 60%),
                           radial-gradient(ellipse 60% 40% at 70% 0%,  rgba(192,112,128,0.08) 0%, transparent 60%)`,
            }} />

            {/* Üst köşe sümbüller */}
            <div style={{ position: "absolute", top: 0, left: 0 }}>
              <WisteriaBotanical side="left" opacity={0.28} />
            </div>
            <div style={{ position: "absolute", top: 0, right: 0 }}>
              <WisteriaBotanical side="right" opacity={0.28} />
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center w-full"
              ...
              <div style={{
                ...
                border: `1.5px solid ${LILAC}22`,
                padding: "10px 10px 0",
                marginBottom: 32,
              }}>
                <div style={{
                  borderRadius: "152px 152px 14px 14px",
                  border: `1px solid ${LILAC}14`,
                  ...
                  <p style={{
                    ...
                    color: LILAC_MD, textTransform: "uppercase", marginBottom: 18,
                  }}>Nişan Davetiyesi</p>

                  <p style={{
                    fontFamily: "var(--font-dancing),cursive",
                    fontSize: "clamp(3rem,10vw,5rem)",
                    color: WARM, lineHeight: 1.05, marginBottom: 0,
                  }}>{isim1}</p>
                  <p style={{
                    fontFamily: "var(--font-dancing),cursive",
                    fontSize: "clamp(1.4rem,4.5vw,2.2rem)",
                    color: PINK, lineHeight: 1.3,
                  }}>&</p>
                  {isim2 && (
                    <p style={{
                      fontFamily: "var(--font-dancing),cursive",
                      fontSize: "clamp(3rem,10vw,5rem)",
                      color: WARM, lineHeight: 1.05, marginBottom: 20,
                    }}>{isim2}</p>
                  )}

                  <div style={{ maxWidth: 180, margin: "0 auto 16px" }}><FloralDivider /></div>

                  {tarihKisa && (
                    <p style={{
                      fontFamily: "var(--font-cormorant),serif",
                      fontSize: 11, letterSpacing: "0.28em",
                      color: `${WARM}60`, marginBottom: 4,
                    }}>{tarihKisa}</p>
                  )}
                </div>
              </div>
              ...
              <div style={{
                ...
                <p style={{
                  ...
                  color: `${WARM}45`, letterSpacing: "0.12em", marginTop: 10,
                }}>Mühüre dokun</p>
              </div>
            </div>

            {/* Alt sümbül sarkaçları */}
            <div style={{ position: "absolute", bottom: 0, left: 0, transform: "scaleY(-1) scaleX(1)", opacity: 0.15 }}>
              <WisteriaBotanical side="left" />
            </div>
            <div style={{ position: "absolute", bottom: 0, right: 0, transform: "scaleY(-1) scaleX(-1)", opacity: 0.15 }}>
              <WisteriaBotanical side="left" />
            </div>
          </div>

          {/* ── Video overlay — her zaman DOM'da ── */}
          <div style={{
            ...
            <div style={{
              ...
              {isimlerGorunur && (
                <svg width="160" height="24" viewBox="0 0 160 24" fill="none"
                  style={{ marginBottom: 20, animation: "lineSvgDraw 1.2s ease forwards" }}>
                  <path d="M0 12 C30 4 50 20 80 12 C110 4 130 20 160 12"
                    stroke={PINK_LT} strokeWidth="1" fill="none" strokeDasharray="400" strokeDashoffset="0" opacity="0.8" />
                  <circle cx="80" cy="12" r="3" fill={PINK_LT} opacity="0.9" />
                  <circle cx="40" cy="12" r="1.5" fill={PINK_LT} opacity="0.6" />
                  <circle cx="120" cy="12" r="1.5" fill={PINK_LT} opacity="0.6" />
                </svg>
              )}
              <p style={{
                fontFamily: "var(--font-dancing),cursive",
                ...
                textShadow: "0 2px 32px rgba(0,0,0,0.5), 0 0 60px rgba(0,0,0,0.25)",
      ...
      {acildi && (
        <div style={{ background: BG, minHeight: "100vh", overflowX: "hidden" }}>
          ...
          <section style={{
            ...
            {/* Köşe sümbüller */}
            <div style={{ position: "absolute", top: 0, left: 0 }}>
              <WisteriaBotanical side="left" opacity={0.22} />
            </div>
            <div style={{ position: "absolute", top: 0, right: 0 }}>
              <WisteriaBotanical side="right" opacity={0.22} />
            </div>
            <div style={{ position: "absolute", bottom: 0, left: 0, transform: "scaleY(-1)", opacity: 0.12 }}>
              <WisteriaBotanical side="left" />
            </div>
            <div style={{ position: "absolute", bottom: 0, right: 0, transform: "scaleY(-1) scaleX(-1)", opacity: 0.12 }}>
              <WisteriaBotanical side="left" />
            </div>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
                  <svg width="28" height="28" viewBox="0 0 28 28">
                    {[0, 72, 144, 216, 288].map((deg, i) => {
                      const rad = (deg - 90) * Math.PI / 180;
                      const px = 14 + Math.cos(rad) * 7;
                      const py = 14 + Math.sin(rad) * 7;
                      return <ellipse key={i} cx={px} cy={py} rx="3.5" ry="5.5" fill={PINK}
                        opacity="0.45" transform={`rotate(${deg}, ${px}, ${py})`} />;
                    })}
                ...
                <p style={{ fontFamily: "var(--font-dancing),cursive", fontSize: "clamp(3.6rem,12vw,5.5rem)", color: WARM, lineHeight: 1, marginBottom: 2 }}>
                  {isim1}
                </p>
                <p style={{ fontFamily: "var(--font-dancing),cursive", fontSize: "clamp(1.6rem,5vw,2.4rem)", color: PINK, lineHeight: 1.4 }}>
                  &
                </p>
                {isim2 && (
                  <p style={{ fontFamily: "var(--font-dancing),cursive", fontSize: "clamp(3.6rem,12vw,5.5rem)", color: WARM, lineHeight: 1, marginBottom: 8 }}>
                    {isim2}
                  </p>
                )}
            ...
            <div style={{ marginTop: 48, textAlign: "center" }}>
              ...
              <div style={{
                width: 26, height: 26,
                border: `1px solid ${LILAC}28`, borderRadius: "50%",
                display: "flex", alignItems: "center", justifyContent: "center",
          ...
          {/* ════ BÖLÜM 2 — POLAROİD ANILARI ════ */}
          {davetiye.albumAktif && (
            <section style={{ padding: "80px 24px 100px", textAlign: "center", background: BG_SOFT }}>
              <p style={{ fontFamily: "var(--font-cormorant),serif", fontSize: 10, letterSpacing: "0.36em", color: LILAC, textTransform: "uppercase", marginBottom: 12 }}>
                Bizim Hikayemiz
              </p>
              <p style={{ fontFamily: "var(--font-dancing),cursive", fontSize: "clamp(2.4rem,7.5vw,3.8rem)", color: WARM, lineHeight: 1.1, marginBottom: 20 }}>
                En Güzel Anılar
              </p>
              ...
              <p style={{ fontFamily: "var(--font-dancing),cursive", fontSize: 17, fontStyle: "italic", color: `${WARM}50` }}>
          ...
          <section style={{ padding: "80px 24px", textAlign: "center", background: BG }}>
            ...
            <p style={{ fontFamily: "var(--font-dancing),cursive", fontSize: "clamp(2.2rem,7vw,3.6rem)", color: WARM, marginBottom: 36 }}>
              Nerede &amp; Ne Zaman?
            </p>
            ...
            <div style={{
              ...
              padding: "36px 32px",
              boxShadow: `0 8px 40px rgba(122,82,160,0.07), 0 2px 8px rgba(0,0,0,0.04)`,
            ...
            {davetiye.mekan && (
              <div style={{
                ...
                border: `1px solid ${LILAC}20`,
                boxShadow: `0 6px 24px rgba(122,82,160,0.10)`,
              }}>
                <iframe
          ...
          {davetiye.mesaj && (
            <section style={{ padding: "70px 24px 80px", textAlign: "center", background: BG_SOFT }}>
              <div style={{ maxWidth: 400, margin: "0 auto" }}>
                {/* Tırnak işareti çiçek versiyonu */}
          ...
          <section style={{ padding: "80px 24px", background: BG }}>
            <div style={{ maxWidth: 480, margin: "0 auto" }}>
              <div style={{
                ...
                <p style={{ fontFamily: "var(--font-cormorant),serif", fontSize: 10, letterSpacing: "0.32em", color: LILAC, textAlign: "center", textTransform: "uppercase", marginBottom: 10 }}>
                  Katılım Bildirimi
                </p>
                <p style={{ fontFamily: "var(--font-dancing),cursive", fontSize: "clamp(1.8rem,6.5vw,2.8rem)", color: WARM, textAlign: "center", lineHeight: 1.1, marginBottom: 22 }}>
                  Gelecek misiniz?
                </p>
          ...
          <footer style={{ background: BG_SOFT, padding: "40px 24px 80px", textAlign: "center", position: "relative", overflow: "hidden" }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 20, opacity: 0.2 }}>
              <WisteriaBotanical side="left" />
