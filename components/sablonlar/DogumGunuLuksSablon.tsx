      ...
      <section id="mekan" style={{ padding:"80px 24px", textAlign:"center", background:BG_MED }}>
        ...
        {davetiye.mekan && (
          <a href={`https://maps.google.com/?q=${encodeURIComponent(davetiye.mekan)}`} target="_blank" rel="noopener noreferrer"
            style={{ display:"block", textDecoration:"none", maxWidth:560, margin:"0 auto 28px", borderRadius:12, overflow:"hidden", border:`1px solid ${GOLD}22`, boxShadow:"0 8px 32px rgba(0,0,0,0.4)" }}>
            <div style={{ height:220, position:"relative", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:12, padding:"32px 24px", background:BG_MED }}>
              <div style={{ position:"absolute", inset:0, pointerEvents:"none", backgroundImage:`linear-gradient(${GOLD}08 1px,transparent 1px),linear-gradient(90deg,${GOLD}08 1px,transparent 1px)`, backgroundSize:"28px 28px" }} />
              <div style={{ position:"relative", textAlign:"center" }}>
                <div style={{ fontSize:32, marginBottom:10 }}>📍</div>
                <p style={{ fontFamily:"var(--font-cormorant),serif", fontSize:15, color:GOLD, letterSpacing:"0.05em", lineHeight:1.5, marginBottom:16 }}>{davetiye.mekan}</p>
                <div style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"10px 24px", borderRadius:8, border:`1px solid ${GOLD}44`, color:GOLD, fontSize:11, letterSpacing:"0.2em", textTransform:"uppercase", fontFamily:"var(--font-cormorant),serif", background:"rgba(0,0,0,0.25)" }}>Yol Tarifi Al</div>
              </div>
            </div>
          </a>
        )}
...
function RsvpFormKrem({ davetiyeId }: { davetiyeId: string }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop:22, border:`1px solid ${GOLD}24`, borderRadius:10, overflow:"hidden", background:"rgba(255,184,77,0.08)" }}>
        <button type="button" onClick={() => setEkBilgiAcik(!ekBilgiAcik)} style={{
          ...
          <span>
            ...
            <span style={{ display:"block", fontSize:11, color:"#7A5A28", marginTop:2 }}>Diyet tercihi veya şarkı dileği</span>
          </span>
          ...
          <div style={{ padding:"0 14px 14px", borderTop:`1px solid ${GOLD}18` }}>
            {form.katilim === "evet" && (
              <div style={{ marginTop:14 }}>
                <label style={labelStyle}>Diyet Tercihleri <span style={{ textTransform:"none", letterSpacing:0, fontSize:10, color:"#7A5A28" }}>(isteğe bağlı)</span></label>
                <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginTop:8 }}>
                  {[{ k:"vegan", l:"🌱 Vegan" },{ k:"vejetaryen", l:"🥗 Vejetaryen" },{ k:"glutensiz", l:"🌾 Glutensiz" },{ k:"laktozsuz", l:"🥛 Laktozsuz" }].map(opt => (
                    <button key={opt.k} type="button" onClick={() => toggleDiyet(opt.k)} style={{
                      ...
                      color: secilenDiyet.includes(opt.k) ? BG : "#7A5A28",
