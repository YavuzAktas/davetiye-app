...
function FotoKart({
  ...
  return (
    <div className="flex items-start gap-4 p-4 rounded-2xl border border-gray-100 hover:border-gray-200 transition-colors bg-white">
      <div className="relative w-20 h-20 rounded-xl overflow-hidden shrink-0 bg-gray-100 cursor-zoom-in" onClick={onLightbox}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${foto.onaylandi ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}>
      ...
      <div className="flex gap-2 shrink-0">
        <button
          ...
          disabled={yukleniyor}
          className={`text-xs font-semibold px-3 py-1.5 rounded-xl transition-all disabled:opacity-50 ${foto.onaylandi ? "bg-gray-100 text-gray-500 hover:bg-gray-200" : "text-white hover:opacity-90"}`}
          style={!foto.onaylandi ? { backgroundColor: renk } : {}}
        >
          {yukleniyor ? "..." : foto.onaylandi ? "Geri Al" : "Onayla"}
        </button>
        <button onClick={onSil} disabled={yukleniyor} className="text-xs font-semibold px-3 py-1.5 rounded-xl bg-red-50 text-red-500 hover:bg-red-100 transition-all">
          Sil
        </button>
      </div>
    </div>
  ...
  return (
    <div className="p-4 rounded-2xl border border-gray-100 hover:border-gray-200 transition-colors bg-white">
      <div className="flex items-start gap-3 mb-3">
        <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold shrink-0" style={{ backgroundColor: renk + "18", color: renk }}>
          {ani.yazarAd[0]?.toUpperCase()}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-gray-800 text-sm">{ani.yazarAd}</span>
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${ani.onaylandi ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}>
        ...
        <div className="flex gap-2 shrink-0">
          <button
            onClick={async () => { setYukleniyor(true); await onToggle(); setYukleniyor(false); }}
            disabled={yukleniyor}
            className={`text-xs font-semibold px-3 py-1.5 rounded-xl transition-all disabled:opacity-50 ${ani.onaylandi ? "bg-gray-100 text-gray-500 hover:bg-gray-200" : "text-white hover:opacity-90"}`}
      ...
      <p className="text-sm text-gray-600 leading-relaxed bg-gray-50 rounded-xl p-3">{ani.icerik}</p>
    </div>
  );
}
